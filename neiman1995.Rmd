...
```{r sed, echo=FALSE}
sed <- function(amount_of_variants) {
  pi <- runif(amount_of_variants)
  pj <- runif(amount_of_variants)
  pi <- pi / sum(pi)
  pj <- pj / sum(pj)
  sum((pi - pj)^2)
}
...
renderPlot({
  tibble::tibble(
    ...
    sed_min = sapply(amount_of_variants, function(x) {
      min(replicate(500, sed(x)))
    }),
    sed_max = sapply(amount_of_variants, function(x) {
      max(replicate(500, sed(x)))
    }),
...
```{r group_drift_simulation, echo=FALSE}
inputPanel(
...
```{r sed_sim, echo=FALSE}
sed_2 <- function(pi, pj) {
  pi <- pi / sum(pi)
  pj <- pj / sum(pj)
  sum((pi - pj)^2)
}
...
renderPlot({
  ...
  pop_devel_sum <- group_drift_simulation_data()
  ...
  sed_res <- purrr::map2_dbl(A, B, function(a, b) {sed_2(a, b)})
