...
group_drift_simulation_data_3 <- eventReactive(
  input$run_button_group_drift_simulation_iter_2, {
    ...
    res_list <- list()
    for (i in 1:input$sim_runs_2) {
      res_list[[i]] <- calculate_sed_for_group_drift_simulation_result(
        group_drift_simulation(
          input$k_group_3, 
          input$N_group_3, 
          input$t_final_group_3, 
          input$mi_3
        ),
        sim_run = i
      ) 
    ...
    Mk_2 <- function(k) {
      ...
      #M[] <- 0.2
      M[1,] <- c(1 - input$mi_3, input$mi_3)
      M[2,] <- c(input$mi_3, 1 - input$mi_3)
      M
    }
    
    mixed_list[[2]] <- multi_group_matrizes(input$N_group_3, input$t_final_group_3, Mk_2)
