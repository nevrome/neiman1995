...
The document mirrors the structure in the article and should be read in conjunction with it. It can be used to deepen the understanding of the complex variable relations, but it doesn't attempt to retell the story. The small descriptive texts are compiled from the article and may contain sentences copied verbatimly.
...
inputPanel(
  ...
  sliderInput("N_e_drift_simulation", label = "Ne",
...
renderPlot({
  ...
  Ft <- sapply(timesteps, function(x) { Ft(F0, Ne, x) })
...
inputPanel(
  sliderInput("k_drift_simulation_with_innovation", label = "k for t == 0",
               min = 5, max = 15, value = 10, step = 1),
  sliderInput("N_drift_simulation_with_innovation", label = "N",
               min = 10, max = 100, value = 20, step = 10),
...
drift_simulation_with_innovation_data <- eventReactive(
  input$run_button_drift_simulation_with_innovation, {
    ...
    N <- input$N_drift_simulation_with_innovation
