...
renderPlot({
  ...
  pop_devel_sum %>%
    ggplot() +
    geom_area(aes(time, individuals_with_variant)) +
    facet_grid(variant ~ .)
})
```
...
inputPanel(
  sliderInput("k_2", label = "k",
               min = 1, max = 10, value = 10, step = 1),
  sliderInput("N_2", label = "N",
               min = 10, max = 100, value = 20, step = 10),
  sliderInput("mu_sim", label = "μ",
               min = 0, max = 0.005, value = 0.0005, step = 0.0005)
)
...
renderPlot({
  ...
  for (p1 in timesteps) {
    ...
    # innovation
    innovate_here <- sample(c(TRUE, FALSE), length(pop_new$variant), prob = c(mu_sim, 1 - mu_sim), replace = T)
