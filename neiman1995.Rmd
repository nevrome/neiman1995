...
purrr::walk(list.files("./document_code", full.names = T), function(x) {source(x)})
```
...
The first simulation shows the effect of drift in a population of $N$ individuals with $k$ variants over $t$ time steps. Drift is sampling error that accompanies all forms of cultural transmission and causes individual variants to vanish or dominate randomly. In the end one variant usually eradicates all the others. The speed with which the variation is destroyed increases as the population size decreases.  
