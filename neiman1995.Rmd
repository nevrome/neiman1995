...
The real world is better represented in terms of more than two groups learning from another. The multi-group case can be despribed in a matrix based model. The visualization for this model can be found in the next segment "Diversity and Distance for Individual Assemblages". 
...
The matrix of variances and covariances $\mathbf{V}$ can be converted to a matrix of squared Euclidian distances $d_{ij}^{2}$ (or $\mathbf{SED}$).
